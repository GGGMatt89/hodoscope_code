...
int main (int argc,char ** argv)
{
  ...
  //Initializing vectors for saving time datas
  std::vector <int> trigger_number_vec, hit_time_rec_vec;
  ...
  TH1F *time = new TH1F("time info", "time info", 10000, 40, 10040);
  ...
  if(cnt>0){
    ...
    std::cin>>file_to_an;
    if(file_to_an==-1){file_to_an = cnt - 1;}
    for(int fileAn = 0; fileAn < file_to_an; fileAn++){
      ...
      if(fp){
        ...
        if(file_beg.file_beg_id==beginning_file_id){
          ...
          std::cout<<"PROBLEM IN READING FILE HEADER -> CHECK THE FILE STRUCTURE! "<<std::endl;
          return 0;
        }
        for(int Nevnts = 0; Nevnts<file_beg.Ntot_events; Nevnts++){
          ...
          trigger_number_vec.push_back(event_id.trigger_number);
            ...
            cluster->Fill(data_struct.modules_num);//filling cluster plot
	          if(data_struct.mode_num==7){
	              ...
	              unpack_data((unsigned char *)hit_structure_mode0, data_fiber);
                      ...
                      std::cout << hit_time_rec_vec.size()<<std::endl;
		      time->Fill(data_fiber.hit_time_rec);
