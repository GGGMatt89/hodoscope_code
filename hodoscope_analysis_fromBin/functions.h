...
inline void pack_fileEnd(unsigned char* buf, uint16_t id, uint16_t evnt_n, uint32_t oct_n){

  packi16((unsigned char*)(buf), id);
  packi16((unsigned char*)(buf+2), evnt_n);
  packi32((unsigned char*)(buf+4), oct_n);

}

inline void unpack_fileend(unsigned char* buf, FILEEND &file_end){
  file_end.file_end_id = unpacki16((unsigned char *)(buf));
  file_end.evnt_number = unpacki16((unsigned char *)(buf+2));
  file_end.Ntot_oct = unpacki32((unsigned char *)(buf+4));
}
