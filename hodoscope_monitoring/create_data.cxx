...
int main(int argc,char ** argv){
  ...
  char test_fileHead[8];//Buffer for file header
  char test_eventHead[11];//Buffer for evet header
  char test_hitHead[8];//Buffer for data main part
  ...
  for(int files = 0; files<N_files; files++){
    for(ll = 0; ll<ev_in_file; ll++){
      ...
      pack_dataMain((unsigned char *)test_hitHead, (uint16_t)0x00EB, (uint8_t)25, (uint32_t)ll,  (uint8_t)0, (uint8_t)inv_modules);
      //Write event on file
