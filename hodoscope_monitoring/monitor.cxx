...
int main (int argc,char ** argv)
{
  ...
  while(true){//Main while on events and files after first while used for inizialization
    //if(fp.eof()){usleep(1000000);}//wait if we are at the end of a file
    ...
    if(N_event>=N_ev_toRead || fp.eof()){//if we are reading more events than requested or we are at the end of the file
      ...
      //indice++;
      //if(indice>20){usleep(2000000);indice=0;}                //  usleep to allow the user to interact with the plots (ex: zoom)
      sss.str("");
      sss << path;
    ...
    if(fp){
      ...
      if(data_struct.modules_num>0){
		  ...
		  //std::cout<<"Touched fiber N "<<data_fiber.N_fiber_rec<<std::endl;
          ...
          if(molt_X>0){
            //std::cout<<"checking X multiplicity"<<std::endl;
            fin_molt_X = check_mult(vec_x, 3, fin_pos_x);
            //std::cout<<"final position "<<fin_pos_x<<" old mult "<<molt_X<<"  new mult "<<fin_molt_X<<std::endl;
            //for(int xxx = 0; xxx<vec_x.size(); xxx++){
              //std::cout<<vec_x[xxx]<<std::endl;
            //}
          }else{
            fin_molt_X = molt_X;
          }
          ...
          if(molt_Y>0){
            //std::cout<<"checking Y multiplicity"<<std::endl;
            fin_molt_Y = check_mult(vec_y, 3, fin_pos_y);
            //std::cout<<"final position "<<fin_pos_y<<" old mult "<<molt_Y<<"  new mult "<<fin_molt_Y<<std::endl;
            //for(int yyy = 0; yyy<vec_y.size(); yyy++){
              //std::cout<<vec_y[yyy]<<std::endl;
            //}
          }else{
            fin_molt_Y = molt_Y;
          }
          ...
          if(fin_molt_X>0){
            cluster_X->Fill(fin_molt_X);
            cog_X->Fill(temp_pos_X);
            flag_X = true;
          }
          if(fin_molt_Y>0){
	          cluster_Y->Fill(fin_molt_Y);
